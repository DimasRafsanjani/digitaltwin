// ============================================================
    ...
    // --- Last Known Real-Time Sensor Data ---
    const realData = {
        moisture: 85,
        tds: 600,
        light: 12000,
    ...
    const bind = (id, k) => document.getElementById(id).addEventListener('input', e => {
        ...
        // Tampilkan tombol reset data riil karena pengguna telah memanipulasi slider (masuk mode simulasi)
    ...
    // --- LOGIKA HITUNG UMUR OTOMATIS BERDASARKAN TANGGAL TANAM ---
    function updateAgeFromPlantingDate(dateStr) {
        if (!dateStr) return;
        ...
        calculatedAge = THREE.MathUtils.clamp(calculatedAge, 1, 120);
        
        S.age = calculatedAge;
        document.getElementById('slider-age').value = calculatedAge;
        buildPlant();
        updateUI();
    }
    ...
    // Membuka pop-up kalender bawaan browser saat kolom diklik di mana saja
    dateInput.addEventListener('click', () => {
        try {
            dateInput.showPicker();
        } catch (err) {
            console.log("[UI] Browser tidak mendukung showPicker secara langsung:", err);
        }
    });
    ...
    dateInput.addEventListener('change', async e => {
        ...
        if (token) {
            try {
                ...
                console.error("[API] Gagal menyimpan tanggal tanam:", err);
            }
        }
    });

    // Inisiasi awal: Load dari localStorage
    const savedDate = localStorage.getItem('planting_date');
    if (savedDate) {
        dateInput.value = savedDate;
        updateAgeFromPlantingDate(savedDate);
    } else {
        // Jika belum disetel, default tanggal tanam adalah 60 hari yang lalu (umur 60 hari)
    ...
    document.getElementById('toggle-soil').addEventListener('click', e => {
        ground.visible = !ground.visible; e.target.classList.toggle('active');
    });
    document.getElementById('reset-cam').addEventListener('click', () => {
        cam.position.set(0, 4, 10); ctrl.target.set(0, 2, 0);
    });

    // Event Listener untuk Tombol Reset ke Data Riil
    document.getElementById('btn-reset-real').addEventListener('click', () => {
        // Salin data sensor riil terakhir kembali ke state aktif
        ...
        // Hitung ulang umur kronologis berdasarkan tanggal tanam
        const savedDate = localStorage.getItem('planting_date');
        if (savedDate) {
            updateAgeFromPlantingDate(savedDate);
        }

        // Sinkronkan posisi slider UI kembali ke nilai sensor riil
        ...
        document.getElementById('btn-reset-real').style.display = 'none';
    });
    ...
    function initWebSocket() {
        ...
        socket.on("sensorUpdate", (data) => {
            ...
            // Selalu simpan data sensor riil terakhir di latar belakang
            ...
            // Hanya perbarui visual 3D aktif jika pengguna TIDAK sedang dalam mode simulasi manual
    ...
    const checkAuth = async () => {
        ...
        if (token) {
            ...
            // Ambil sinkronisasi pengaturan (tanggal tanam) dari database server
            try {
                ...
                if (res.ok) {
                    ...
                    if (data.planting_date) {
                        ...
                        dateInput.value = data.planting_date;
    ...
    // Cek auth saat pertama kali render
    checkAuth();